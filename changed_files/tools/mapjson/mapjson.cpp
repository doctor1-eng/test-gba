    ...
    for (auto &layout : layouts_data["layouts"].array_items()) {
        ...
        if ((version == "emerald" && layout_version != "emerald")
         || (version == "firered" && layout_version != "frlg"))
            continue;
        // Kanto Saison 1 (suite 32) : un layout tagué "emerald" (donc compilé normalement dans ce
        // build) peut demander le format d'attributs de metatile FRLG (isFrlg=TRUE, boundary
        // primaire/secondaire 640/640/7 au lieu de 512/512/6, bordure de taille variable) via le
        // champ optionnel "metatile_format": "frlg". Sert a reutiliser un tileset FRLG dupliqué
        // hors du garde #if IS_FRLG (headers.h) sans perdre la fidelite de ses metatiles/palettes/
        // attributs au-dela des bornes Emerald standard, et sans que le layout soit exclu de la
        // compilation par le filtre layout_version ci-dessus (qui reste inchangé). Voir
        // docs/TECHNICAL_ARCHITECTURE.md, section "Tilesets Kanto FRLG dans un hack Emerald".
        string metatile_format = json_to_string(layout, "metatile_format", true);
        bool useFrlgFormat = (layout_version == "frlg") || (metatile_format == "frlg");
