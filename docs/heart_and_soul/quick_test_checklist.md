...
- [ ] **Nouveau** : juste avant le texte du carnet, un Grunt Team Rocket distinct (sprite visible) traverse l'écran en courant, une rangée au-dessus de vous, puis disparaît — c'est lui qui vient de laisser tomber le carnet, avant que le texte du carnet ne s'affiche.
- [ ] **Nouveau** : après le texte du carnet du Grunt, le joueur marche automatiquement (quelques cases, visible à l'écran) jusqu'à une PNJ (sprite visible) représentant des réfugiés cachés, **accompagnée d'un enfant (sprite visible)** à côté d'elle, avant que le texte des réfugiés ne s'affiche.
- [ ] Choix réfugiés (guider / cacher) : les deux options fonctionnent, message de résultat cohérent. Une fois le choix fait, la PNJ réfugiée **et l'enfant à côté d'elle** disparaissent tous les deux.
- [ ] **Nouveau, corrigé une 2e fois** : le joueur marche ensuite jusqu'à un Pokémon sauvage blessé (sprite visible, un Growlithe, repositionné une seconde fois sur une case confirmée sèche — la première correction était encore dans l'eau) avant que le texte du Pokémon blessé ne s'affiche.
- [ ] Choix Pokémon blessé (aider / continuer) : les deux options fonctionnent. Une fois le choix fait, le sprite du Pokémon disparaît.
- [ ] Le joueur marche une dernière fois vers le « port » (quelques cases, chemin recalculé pour rester sur la terre ferme), le texte de l'objet trouvé (bracelet) s'affiche, puis fondu au noir et **téléportation automatique dans l'intérieur d'un bateau** (nouvelle carte `MAP_CINNABAR_ISLAND_BOAT_CABIN_HNS`, pas la Route 21 en extérieur).
- [ ] **Nouveau, corrigé** : dans l'intérieur du bateau (une seule pièce), un bref texte annonce la tempête (son de tonnerre), puis un marin (sprite visible) marche vers vous et vous parle — le joueur doit être en **mode normal (à pied)**, jamais en mode Surf, puisque toute la scène se déroule maintenant à l'intérieur d'une carte indoor —, avant un nouveau fondu au noir et la **téléportation automatique à Pallet Town**.
