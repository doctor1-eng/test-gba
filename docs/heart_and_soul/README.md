...
## Contexte
Fork de **pokeemerald-expansion**, région de Kanto en monde ouvert. L'histoire complète, les personnages, les mécaniques narratives et les systèmes (réputation, conviction des lieutenants) sont détaillés dans `docs/histoire.md`. Ce fichier README sert de point d'entrée : lis-le en premier, puis réfère-toi à `docs/histoire.md` pour tout détail narratif, et à `scripts/` pour les templates Poryscript déjà rédigés (génériques, à adapter aux vraies constantes du repo).
...
## Ordre de priorité (tous les blocs sont scriptés — templates génériques à adapter)
...
1. Explorer le repo pour identifier les vraies constantes (maps, espèces, flags/vars déjà utilisés) et les conventions de nommage du projet.
2. Adapter `scripts/intro_choix_type.pory` en conséquence, en dupliquant la structure pour les 17 types manquants (seul Feu est fait en exemple).
3. Adapter `scripts/cinnabar_acte1.pory` (vérifier notamment la gestion de visibilité de Blaine et les IDs de map réels).
4. Suivre l'ordre narratif (Acte I → Acte V → post-game) pour adapter les fichiers restants, en vérifiant à chaque étape que les flags/vars utilisés sont cohérents entre les fichiers (ex. `FLAG_TERRENCE_CONVAINCU` posé dans `terrence_conviction.pory` est relu dans `epilogues.pory`).
5. Me signaler toute incohérence trouvée entre les templates et la structure réelle du repo plutôt que de la corriger silencieusement — certains choix (ex. names de flags, seuils numériques) sont issus de décisions de conception documentées dans `docs/histoire.md` et ne doivent pas être modifiés sans validation.
