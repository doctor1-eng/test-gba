...
|---|---|---|---|---|
| Cinnabar (île) | `CinnabarIsland_hns` | `Route20_hns`, `Route21_hns` (→ Johto ; **entièrement en eau, Surf requis, non praticable en tout début de partie** — cause du bug bloquant du retour de test n°5), warp Pokémon Center, **+ 3 warps bâtiments (Gym/Manoir/Labo), voir ci-dessous**, **+ 1 warp grotte (`(54,15)`), voir ci-dessous** | Oui — **complétée** | Fuite/retour gérés par `warp` scripté (voir implementation_notes.md). Warps vers Gym/Manoir/Labo **posés et fonctionnels**, pointant vers 3 nouvelles maps `_Hns` natives (voir "Bâtiments de Cinnabar" ci-dessous). PNJ Blaine/Blue existants réécrits (voir conflit narratif). **Nouveau (2026-09-13b)** : `MAP_CINNABAR_ISLAND_CAVE_HNS`, grotte des « derniers habitants » (histoire.md section 8), voir `implementation_notes.md` section dédiée — construite après décision explicite de revenir sur le choix documenté plus tôt de ne pas créer cette carte. |
